    ...
    if (isFunctionComponent(type)) {
      ...
      const peelResult = yield* Effect.either(
        Effect.gen(function* () {
          ...
          yield* hydrateVElementToDOM(
            maybeFirst.value,
            domNode,
            runtime,
            yield* Ref.get(contentScopeRef),
            path,
          );
      ...
      if (
        ...
        const finalState = yield* Effect.iterate(
          {
            index: 0,
            cursor: Option.fromNullable(domNode.nextSibling as Node),
          },
          {
            while: (state) => {
              ...
              return Option.match(state.cursor, {
                ...
                onSome: (node) => {
                  ...
                  if (
                    ...
                    (node as Comment).data.includes("/fibrae:sus")
                  ) {
                    return false;
                  }
                  return true;
        ...
        return Option.match(finalState.cursor, {
          ...
          onSome: (maybeClosingMarker) => {
            if (
              maybeClosingMarker.nodeType === Node.COMMENT_NODE &&
              (maybeClosingMarker as Comment).data.includes("/fibrae:sus")
            ) {
        ...
        return yield* Effect.fail(
          new HydrationMismatch({
            expected:
              "Suspense comment marker (<!--fibrae:sus:resolved--> or <!--fibrae:sus:fallback-->)",
            actual:
              domNode.nodeType === Node.COMMENT_NODE
