...
const StreamCounter = () => {
  const items = [
    ...
    <div data-cy="stream-status">
      <p style="color: #4ade80;">Complete!</p>
    </div>,
  ];
...
const TodoList = () =>
  Effect.gen(function* () {
    ...
    const addTodo = (currentInput: string) => {
      return Effect.sync(() =>
        registry.update(todosAtom, (list: string[]) => list.concat(currentInput)),
      );
    };

    const removeTodo = (todoToRemove: string) => {
      return Effect.sync(() =>
        registry.update(todosAtom, (list: string[]) =>
...
const Subtitle = ({ children }: { children: VNode | string }) => (
  <p style="text-align: center; color: #999;">{children}</p>
);
...
const DebouncedSearch = () =>
  Effect.gen(function* () {
    ...
    const results =
      currentDebouncedQuery.length >= 2
        ? [
            `Result 1 for "${currentDebouncedQuery}"`,
            `Result 2 for "${currentDebouncedQuery}"`,
            `Result 3 for "${currentDebouncedQuery}"`,
          ]
        : [];
    ...
    return (
      <div data-cy="debounced-search">
        ...
        />
        <p style="color: #999;">
          Query: <strong>{currentQuery}</strong> | Debounced:{" "}
          <strong>{currentDebouncedQuery || "(none)"}</strong>
          {searching && " | 🔄 Searching..."}
        </p>
          ...
          {results.length > 0 ? (
            results.map((result, i) => (
              <li
                ...
                style="padding: 0.5rem; background: #333; margin: 0.25rem 0; border-radius: 4px;"
              >
                {result}
              </li>
            ))
            ...
            <li style="color: #999; font-style: italic;">
              {currentDebouncedQuery.length > 0
...
class ThemeService extends Effect.Service<ThemeService>()("ThemeService", {
  ...
  effect: Effect.gen(function* () {
    ...
    return {
      ...
      toggleTheme: () =>
        Effect.sync(() =>
          registry.update(themeAtom, (t: "light" | "dark") => (t === "light" ? "dark" : "light")),
        ),
    };
  }),
}) {}
...
const UserList = () =>
  Effect.gen(function* () {
    ...
    return (
      <div
        data-cy="user-list"
        style={`padding: 1rem; background: ${bgColor}; color: ${textColor}; border-radius: 8px;`}
      >
...
Effect.gen(function* () {
  ...
  const counterContainer = createSection(
    "Example 1: Simple Counter",
    "Basic reactive state with Atom.make()",
  );
  const staticContainer = createSection("Example 2: Static Components", "Components without state");
  const streamContainer = createSection(
    "Example 3: Stream with Suspense",
    "Progressive rendering with fallback",
  );
  const todoContainer = createSection(
    "Example 4: Todo List with Child Components",
    "Form submission and nested components",
  );
  const searchContainer = createSection(
    "Example 5: Debounced Search 🔬",
    "Testing: Effect.delay in event handlers + multiple atom updates",
  );
  const serviceContainer = createSection(
    "Example 6: Effect Services 🔬",
    "Testing: Shared services with Atoms + async Effect.sleep (1-2 sec delays)",
  );
  const errorContainer = createSection(
    "Example 7: Error Boundaries",
    "Render-time, event, and stream errors",
    "error-container",
  );
  ...
  yield* Effect.fork(
    render(
      <Suspense fallback={<div data-cy="stream-loading"> Loading stream...</div>}>
        <StreamCounter />
      </Suspense>,
      streamContainer,
    ),
      ...
      <>
        ...
        >
          <div>
            <button
              data-cy="fail-event"
              onClick={() => Effect.fail(new TestFailure({ message: "event-crash" }))}
            >
              Fail Event
            </button>
          </div>
        ...
        <ErrorBoundary fallback={() => <div data-cy="fallback-stream">Stream Error</div>}>
          {Stream.concat(
            Stream.succeed(<div data-cy="stream-ok">Stream OK once</div>),
            Stream.fromEffect(
              Effect.delay(Effect.fail(new TestFailure({ message: "stream-crash" })), "300 millis"),
            ),
          )}
        </ErrorBoundary>
        <ErrorBoundary
        ...
        >
          {Stream.fromEffect(Effect.fail(new TestFailure({ message: "stream-crash-immediate" })))}
        </ErrorBoundary>
        <ErrorBoundary
        ...
        >
          <Suspense fallback={<div data-cy="suspense-loading">Loading slow component...</div>}>
            {Stream.fromEffect(
              Effect.delay(
                Effect.fail(new TestFailure({ message: "slow-then-fail" })),
                "200 millis",
              ),
            )}
          </Suspense>
  ...
  // Typed error handling examples — pattern match on _tag
  const boundaryContainer = createSection(
    "Example 8: Typed Error Handling",
    "Pattern match on error._tag for per-type fallbacks",
    "boundary-container",
  );
  ...
  const BoundaryEventFailer = () => (
    <div>
      <button
        data-cy="boundary-fail-event"
        onClick={() => Effect.fail(new TestFailure({ message: "boundary-event-crash" }))}
      >
        Fail Event (boundary)
      </button>
    </div>
  ...
  const BoundaryStreamFailer = () => {
    ...
    const fail = Effect.delay(
      Effect.fail(new TestFailure({ message: "boundary-stream-crash" })),
      "300 millis",
    );
    return Stream.concat(Stream.succeed(ok), Stream.fromEffect(fail));
  };

  // Render typed error handling examples
  yield* Effect.fork(
    render(
      <>
        <ErrorBoundary
          fallback={(e) =>
            e._tag === "RenderError" ? (
              <div data-cy="boundary-fallback-render">Boundary Render Error: {e.componentName}</div>
            ) : (
              <div>Error</div>
            )
          }
        >
          <BoundaryCrashDuringRender />
        </ErrorBoundary>
        <ErrorBoundary
          fallback={(e) =>
            e._tag === "EventHandlerError" ? (
        ...
        <ErrorBoundary
          fallback={(e) =>
            e._tag === "StreamError" ? (
              <div data-cy="boundary-fallback-stream">Boundary Stream Error: {e.phase}</div>
            ) : (
              <div>Error</div>
            )
          }
        >
          <BoundaryStreamFailer />
        </ErrorBoundary>
      </>,
      boundaryContainer,
    ),
