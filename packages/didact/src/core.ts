...
/**
 * Render a Didact component into a container.
 * 
 * If container has child elements, enters hydration mode:
 * - Attaches event handlers to existing DOM
 * - Restores atom state from initialState if provided
 * 
...
const hydrateVElementToDOM = (
  ...
  Effect.gen(function* () {
    ...
    if (isFunctionComponent(type)) {
      ...
      const stream = normalizeToStream(outputResult as VElement | Effect.Effect<VElement> | Stream.Stream<VElement>).pipe(
        Stream.provideContext(contextWithTracking)
      );
...
export function render(
  ...
  const program = (cont: HTMLElement) =>
    Effect.gen(function*() {
      ...
      // Mark container as hydrated/rendered - signals readiness for interaction
      cont.setAttribute("data-hydrated", "true");
