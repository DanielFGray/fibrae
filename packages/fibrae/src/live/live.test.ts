...
const bodyToReadable = (response: HttpServerResponse.HttpServerResponse): ReadableStream<Uint8Array> =>
  Stream.toReadableStream((response.body as any).stream)
    ...
    const response = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        return yield* serve(ch, {
          ...
          source: Ref.get(ref).pipe(Effect.map((n) => new Item({ id: 1, name: n === 0 ? "a" : "b" }))),
          interval: "50 millis",
